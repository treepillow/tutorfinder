...
import { Search, BookOpen, X, DollarSign, Bell } from 'lucide-react';
...
const PROFILE_URL = import.meta.env.VITE_PROFILE_SERVICE || 'http://localhost:5001';
const MATCH_URL   = import.meta.env.VITE_MATCH_SERVICE   || 'http://localhost:5002';
...
export function MatchesPage({ user, onNavigate, onLogout }: MatchesPageProps) {
  ...
  const fetchMatches = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${MATCH_URL}/match/matches/${user.user_id}`);
      if (!res.ok) throw new Error('Failed to fetch matches');
      const data = await res.json();
      const matchList: { match_id: number; other_user_id: number }[] = data;

      const profiles = await Promise.all(
        matchList.map(async m => {
          const pRes = await fetch(`${PROFILE_URL}/profile/${m.other_user_id}`, {
            headers: { Authorization: `Bearer ${user.token}` },
          });
          if (!pRes.ok) return null;
          const p = await pRes.json();
          return { match_id: m.match_id, ...p } as MatchedProfile;
        })
      );
      setMatches(profiles.filter(Boolean) as MatchedProfile[]);
    } catch (e) {
      setError('Could not load matches. Make sure services are running.');
    } finally {
      setLoading(false);
    }
  }, [user]);
    ...
    <div ref={containerRef} className="page-layout">
      ...
      <main className="page-main">
        ...
        <div className="page-content">
          ...
          {loading && <div className="state-msg">Loading matches...</div>}
          {error && <div className="state-msg error">{error}</div>}

          {!loading && !error && filtered.length === 0 && (
            <div className="empty-state">
              <p>No matches yet. Start swiping on the Discover page!</p>
              <button className="btn btn-primary" onClick={() => onNavigate('discover')}>
                Go Discover
              </button>
            </div>
              ...
              {filtered.map(profile => (
                ...
                >
                  <div className="match-card-header">
                    <div className="match-avatar" style={{ background: getGradient(profile.user_id) }}>
                  ...
                  <div className="match-card-body">
                    <h3>{profile.name}</h3>
                    {profile.subject && (
                      <p className="match-subject"><BookOpen size={14} />{profile.subject}</p>
                    )}
                    {profile.price_rate !== null && profile.price_rate !== undefined && (
                      <div className="match-meta">
                        <span><DollarSign size={14} />${profile.price_rate}/hr</span>
                      </div>
                    )}
                    {profile.bio && (
                      <p className="match-bio">{profile.bio.slice(0, 80)}{profile.bio.length > 80 ? '…' : ''}</p>
                    )}
                  </div>
        ...
        <div className="modal-overlay" onClick={() => setSelectedProfile(null)}>
          <div className="profile-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <div className="modal-avatar" style={{ background: getGradient(selectedProfile.user_id) }}>
                {getInitials(selectedProfile.name)}
              </div>
              <div>
                <h2>{selectedProfile.name}</h2>
                {selectedProfile.subject && (
                  <p className="modal-subject">{selectedProfile.subject} {selectedProfile.role}</p>
                )}
              </div>
            </div>
            <div className="modal-body">
              ...
              {selectedProfile.price_rate !== null && selectedProfile.price_rate !== undefined && (
                <div className="modal-stats">
                  <div className="modal-stat">
                    <DollarSign size={18} />
                    <span className="stat-val">${selectedProfile.price_rate}</span>
            ...
            <div className="modal-footer">
              <button className="btn btn-secondary btn-lg" onClick={() => setSelectedProfile(null)}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        .state-msg { text-align: center; padding: var(--space-8); color: var(--text-light); }
        .state-msg.error { color: #ef4444; }
        .empty-state { text-align: center; padding: var(--space-12); color: var(--text-light); display: flex; flex-direction: column; align-items: center; gap: var(--space-4); }
        ...
        .match-bio { font-size: 0.825rem; color: var(--text-light); line-height: 1.5; margin-top: var(--space-2); }
