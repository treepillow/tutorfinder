...
export function RegisterTutorDetails() {
  const navigate = useNavigate();
  const [subjects, setSubjects] = useState<SubjectEntry[]>([
    { id: "1", subject: "", level: "", hourlyRate: "" },
  ]);
  ...
  const addSubject = () => {
    setSubjects([...subjects, { id: Date.now().toString(), subject: "", level: "", hourlyRate: "" }]);
  };
      ...
      <div className="relative z-10 px-8 pb-16 pt-4">
        <div className="w-full max-w-3xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-4xl font-bold tracking-tight text-[#1A2035] mb-2">Tutor Details</h1>
            <p className="text-[#1A2035]/60">Tell us about your teaching expertise</p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-[#1A2035]">Subjects & Rates</h3>
                  <button
                    type="button"
                    onClick={addSubject}
                ...
                {subjects.map((subject, index) => (
                  <div key={subject.id} className="bg-[#F5F3EF] p-5 rounded-2xl space-y-4 border border-[#E8E4DC]">
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-[#1A2035] font-medium">Hourly Rate (SGD)</Label>
                        <Input
                          type="number"
                          value={subject.hourlyRate}
                          onChange={(e) => updateSubject(subject.id, "hourlyRate", e.target.value)}
              ...
              {/* Qualification */}
              <div className="space-y-1.5">
                <Label className="text-[#1A2035] font-medium">Highest Level of Education</Label>
                <Select value={qualification} onValueChange={setQualification} required>
                  <SelectTrigger className="bg-[#F5F3EF] border-[#C8BFAE] text-[#1A2035]">
              ...
              <div className="space-y-3">
                <h3 className="text-lg font-bold text-[#1A2035]">Availability</h3>
