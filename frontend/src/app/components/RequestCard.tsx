...
export function RequestCard({ request, userType, onCancel, onAccept, onReject, onPay }: RequestCardProps) {
  ...
  return (
    <div className="bg-[#E9D8BB] rounded-2xl overflow-hidden">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full p-6 flex items-center justify-between hover:bg-[#C9B08E]/30 transition-colors"
      >
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-gradient-to-br from-[#C9B08E] to-[#7C8D8C] rounded-full flex items-center justify-center text-2xl">
            {userType === "student" ? "👨‍🏫" : "🎓"}
          </div>
      ...
      {isExpanded && (
        <div className="px-6 pb-6 space-y-4">
          <div className="bg-[#FFF2D5] p-4 rounded-xl space-y-3">
            ...
            <div className="flex items-start gap-2 text-[#2F3B3D]">
              ...
              <div className="flex flex-wrap gap-2">
                {request.slots.map((slot: string) => (
                  <span key={slot} className="px-3 py-1 bg-[#E9D8BB] rounded-full text-sm">
                    {slot}
                  </span>
                ))}
              </div>
          ...
          <div className="flex gap-3">
            {userType === "student" && request.status === "pending" && onCancel && (
              <button
                onClick={onCancel}
                className="flex-1 px-4 py-2 bg-white text-[#2F3B3D] rounded-full border-2 border-[#C9B08E] hover:bg-red-50 hover:border-red-200 hover:text-red-600 transition-all duration-300"
              >
                Cancel Request
              </button>
            )}
